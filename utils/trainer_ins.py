...
class BiTrainer(Trainer):
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        if self.args.loss_rate > 0: # need gist loss1
            ...
            # loss1 *= self.args.loss_rate
            loss1 *= (self.args.loss_rate/2)

            if self.args.gradient_accumulation_steps > 1 and not self.deepspeed:
                # deepspeed handles loss1 scaling by gradient_accumulation_steps in its `backward`
            ...
            if self.do_grad_scaling:
                ...
                # loss1 gets scaled under gradient_accumulation_steps in deepspeed
                loss1 = self.deepspeed.backward(loss1)
            else:
                loss1.backward()
            ...
            if self.args.gradient_accumulation_steps > 1 and not self.deepspeed:
                # deepspeed handles loss1r scaling by gradient_accumulation_steps in its `backward`
                loss1r = loss1r / self.args.gradient_accumulation_steps
            
            if self.do_grad_scaling:
                self.scaler.scale(loss1r).backward()
            elif self.use_apex:
                with amp.scale_loss(loss1r, self.optimizer) as scaled_loss:
                    scaled_loss.backward()
            elif self.deepspeed:
                # loss1r gets scaled under gradient_accumulation_steps in deepspeed
                loss1r = self.deepspeed.backward(loss1r)
            else:
                loss1r.backward()
        ...
        if self.args.loss_rate < 1: # need contrastive loss2
            ...
            # loss2 *= (1-self.args.loss_rate)
            loss2 *= ((1-self.args.loss_rate)/2)
            
            if self.args.gradient_accumulation_steps > 1 and not self.deepspeed:
                # deepspeed handles loss2 scaling by gradient_accumulation_steps in its `backward`
            ...
            if self.do_grad_scaling:
                ...
                # loss2 gets scaled under gradient_accumulation_steps in deepspeed
            ...
            if self.args.gradient_accumulation_steps > 1 and not self.deepspeed:
                # deepspeed handles loss2r scaling by gradient_accumulation_steps in its `backward`
                loss2r = loss2r / self.args.gradient_accumulation_steps
            
            if self.do_grad_scaling:
                self.scaler.scale(loss2r).backward()
            elif self.use_apex:
                with amp.scale_loss(loss2r, self.optimizer) as scaled_loss:
                    scaled_loss.backward()
            elif self.deepspeed:
                # loss2r gets scaled under gradient_accumulation_steps in deepspeed
                loss2r = self.deepspeed.backward(loss2r)
            else:
                loss2r.backward()
